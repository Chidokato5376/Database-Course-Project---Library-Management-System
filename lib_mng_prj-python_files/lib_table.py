...
def task_call_view():
    ...
    conn = get_connection()
    try:
        # Retrieve first 10 books to check inventory status
...
def task_call_procedure():
    ...
    try:
        ...
        # With pymysql, fetch results directly from cursor
        ...
        if cursor.description:
            ...
            # Handle fine amount display (convert Decimal to float for better display)
...
def task_execute_query():
    ...
    conn = get_connection()
    try:
        # Query to find top 5 most borrowed books
        # Join 3 tables: Book, Loan, Category
        query = """
        SELECT 
            b.book_id,
            b.title,
